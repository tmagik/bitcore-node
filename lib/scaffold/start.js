...
function start(options) {
  ...
  if (options.customNetwork) {
    bitcore.Networks.add(options.customNetwork);
  }

  if (!bitcore.Networks.get(options.config.network)) {
    console.log('Missing configuration definition for network ' + options.config.network);
    process.exit(1);
  }
