...
function main(servicesPath, additionalServices) {
  ...
  program
    ...
    .option('-e, --exec <full-path>', 'Specify the full path to the bitcoind binary')
    .option('-t, --testnet', 'Enable testnet as the network')
    .option('-n, --network <name>', 'Enable a custom network')
  ...
  program
    ...
    .option('-n, --network <configfile>', 'Specify the path to a custom network config file')
    .action(function(cmd){
      if (cmd.config) {
      ...
      if (cmd.network) {
        configInfo.customNetwork = require(path.resolve(process.cwd(), cmd.network));
      }
      start(configInfo);
    });
