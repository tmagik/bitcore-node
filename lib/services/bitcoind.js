...
Bitcoin.prototype._getDefaultConf = function() {
  return {
    rpcport: this.node.network.rpcport || 8332
  };
};
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  ...
  } else if (this.node.network !== bitcore.Networks.livenet) {
    networkPath = this.node.network.name + '/bitcoin.conf';
  }
  return networkPath;
};
...
Bitcoin.prototype._getNetworkOption = function() {
  ...
  if (this.node.network === bitcore.Networks.testnet) {
    ...
    networkOption = '--' + this.node.network.name;
  }
